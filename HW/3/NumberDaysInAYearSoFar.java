...
public class NumberDaysInAYearSoFar {
  public static void main(String[] args) {
    ...
    fullDaysExpired = dayCurrent - 1;
    // kinda pointless flags for output
    pluralityFlag = fullDaysExpired > 1 ? true : false;
    zeroFlag = fullDaysExpired == 0 ? true : false;
    // output
    if (pluralityFlag) {
      System.out.printf("There have been %d full days so far in the year %d.", fullDaysExpired, year);
    } else {
      if (zeroFlag) {
        System.out.printf("It is the first day the year %d.", year);
      } else {
        System.out.printf("There has been one full day in the year %d.", year);
      }
    }
    if (leapYearFlag) {
